...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		while( i  < length) {//aappeele  => aappeel
			int j = i ;
			while(j < length && vowls.contains(word.substring(j, j + 1))) j++;
			if(i != j){
				count++;
				i = j;
			}else i++;
		}
		if(word.charAt(length - 1) == 'e'){
			if(length == 1) return Math.max(1, count - 1);
			else{
				if(!vowls.contains(word.substring(length - 2, length - 1)))
	...
	public double getFleschScore()
	{
		...
		return 206.835 -
				...
				84.6 * ((double) getNumSyllables() / (double) this.getNumWords());
	}
